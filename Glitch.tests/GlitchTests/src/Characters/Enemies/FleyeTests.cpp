	...
	*/
	...
	public:
		...
		TEST_METHOD(Slime_On_Collision_Begin_Top_Should_Remove_Slime)
		{
			...
			dispatcher.dispatchEvent<OnCollisionBeginEvent>((Event&)OnCollisionBeginEvent(make_shared<Player>(player), make_shared<Fleye>(entity), direction));
			// Assert

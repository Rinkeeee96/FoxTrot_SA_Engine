...
void ContactListenerAdapter::BeginContact(b2Contact* contact) {
	...
	if (result.object1 != nullptr && result.object2 != nullptr) {
		...
		(*dispatcher.get()).dispatchEvent<OnCollisionBeginEvent>((Event&)OnCollisionBeginEvent(result.object1->getObject(), result.object2->getObject(), direction));
	}
}
...
void ContactListenerAdapter::EndContact(b2Contact* contact) {
	...
	if (result.object1 != nullptr && result.object2 != nullptr) {
		...
		(*dispatcher.get()).dispatchEvent<OnCollisionEndEvent>((Event&)OnCollisionEndEvent(result.object1->getObject(), result.object2->getObject(), direction));
	}
};

...
void Overworld::loadButtons() {
	...
	auto* level1TextBtn = new Button(2, ColoredText("Level 1, Score: " + to_string(savegame->getCurrentGameData().levelData[1].score), Color(0, 0, 0)), BIND_FN(onLevel1BtnClick), transSprite, this->dispatcher);
	...
	auto* level2Btn = new Button(3, ColoredText("", Color(255, 255, 255)), BIND_FN(onLevel1BtnClick), defaultBtnSprite, this->dispatcher);
	...
	auto* level2TextBtn = new Button(4, ColoredText("Level 2, Score: " + to_string(savegame->getCurrentGameData().levelData[1].score), Color(0, 0, 0)), BIND_FN(onLevel1BtnClick), transSprite, this->dispatcher);
	level2TextBtn->setWidth(100);
	level2TextBtn->setHeight(20);
	level2TextBtn->setPositionX(900);
	level2TextBtn->setPositionY(760);

	auto* stopBtn = new SecondaryButton(5, "To Main Menu", BIND_FN(onStopBtnClick), this->dispatcher);

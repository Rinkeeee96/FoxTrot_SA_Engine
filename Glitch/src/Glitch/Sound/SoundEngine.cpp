...
namespace Glitch {
	...
	void SoundEngine::onPlayEffect(const string& identifier, int volume = MIX_MAX_VOLUME) {
		if (IdentifierExists(identifier)) {
	...
	void SoundEngine::PlayEffect(const string& identifier, int volume = MIX_MAX_VOLUME) {
	...
	/// @param identifier 
	/// The sound identifier saved when the file has been added
	void SoundEngine::StartLoopedEffect(const string& identifier) {
	...
	void SoundEngine::PlayMusic(int volume = MIX_MAX_VOLUME) {
		if (music != NULL) {
	...
	/// @param volume
	/// The volume to play the music at. Ranges from 0 to 128. Defaults to 128 if not given
	void SoundEngine::PlayMusic(const string& identifier, int volume = MIX_MAX_VOLUME) {
		if (soundPaths.find(identifier) != soundPaths.end()) {
			LoadMusic(identifier);
			Mix_VolumeMusic(volume);
			Mix_PlayMusic(music, LOOP_INDEFINITELY);
